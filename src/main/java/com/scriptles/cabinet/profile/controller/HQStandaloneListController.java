...
public class HQStandaloneListController {
    ...
    @PutMapping("/{listId}/items/{itemId}") public HQListService.ListView updateItem(@AuthenticationPrincipal AuthenticatedHQ actor, @PathVariable UUID listId, @PathVariable UUID itemId, @Valid @RequestBody HQListService.ItemUpdateInput input) { return lists.updateItem(current(actor), listId, itemId, input); }
