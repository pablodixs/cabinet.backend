...
| TMDB collection backfill | Every minute by default | When enabled, queues up to 1 indexed collection per tick and pauses at 20 active catalog jobs. Enabled by default; configurable with `CATALOG_TMDB_BACKFILL_ENABLED`, `CATALOG_TMDB_BACKFILL_POLL_DELAY`, `CATALOG_TMDB_BACKFILL_BATCH_SIZE`, and `CATALOG_TMDB_BACKFILL_MAX_ACTIVE_JOBS`. |
...
O worker de FCM reivindica lotes curtos de `notification_deliveries` com bloqueio de linha, envia depois de confirmar a reivindicação e persiste sucesso ou retry em transação separada. Itens `PROCESSING` com mais de dez minutos são recuperados após reinício. A unicidade por notificação/instalação impede duplicação ao repetir a criação; o provider é chamado fora da transação da ação do usuário. O job diário `MEDIA_RELEASE_NOTIFICATIONS` cria notificações para entradas `PLANNED` cuja mídia tem lançamento hoje e ignora mídias com lembrete local registrado. O envio FCM fica desativado por padrão até `FIREBASE_MESSAGING_ENABLED=true`.
